    ...
    if len(results) > 1:
        ...
        for track_id in results.keys():
            ...
            if track_id in tracking_results and 'bbox' in tracking_results[track_id]:
                ...
                if len(bbox_data) > 0:
                    ...
                    # Calculate motion - multiple metrics
                    # 1. Motion from bounding box (translation motion)
                    bbox_motion = 0
                    if bbox_data.shape[0] > 1:
                        ...
                        bbox_motion = np.sum(displacements)
                    
                    # 2. Motion from translation data (similar to bbox but from SMPL model)
                    trans_motion_total = 0
                    ...
                    # 3. Motion from pose data (joint rotations - captures movement like arm swings, shots)
                    pose_motion_total = 0
                    pose_variance_total = 0
                    if 'pose' in results[track_id] and results[track_id]['pose'].shape[0] > 1:
                        # Extract body pose without global orientation (skip first 3 values)
                        body_pose = results[track_id]['pose'][:, 3:]
                        
                        # Calculate the variance of pose parameters across the entire tracklet
                        # Higher variance indicates more movement in the joints
                        pose_variance = np.var(body_pose, axis=0)
                        pose_variance_total = np.sum(pose_variance)
                        
                        # Also keep the original frame-to-frame difference metric
                        pose_diff = np.diff(body_pose, axis=0)
                        pose_motion = np.linalg.norm(pose_diff, axis=1)
                        pose_motion_total = np.sum(pose_motion)
                    
                    # Normalize pose motion metrics to be on comparable scale with translation motion
                    pose_motion_normalized = 0
                    pose_variance_normalized = 0
                    if 'pose' in results[track_id] and results[track_id]['pose'].shape[0] > 1:
                        # Scale factor to make pose motion comparable to translation motion
                        body_pose_dim = results[track_id]['pose'].shape[1] - 3  # Dimension of body pose (excluding root)
                        pose_motion_normalized = pose_motion_total / (body_pose_dim * 10)
                        # Normalize variance - variance values are typically much smaller than frame differences
                        pose_variance_normalized = pose_variance_total / body_pose_dim
                    
                    # Combined motion metric prioritizes pose changes
                    # This helps identify athletes making significant pose movements
                    translation_motion = max(bbox_motion, trans_motion_total)
                    # Use both pose difference and pose variance for a more robust motion estimate
                    # Pose variance captures the range of movement over the entire tracklet
                    total_motion = translation_motion + (pose_motion_normalized * 1.5) + (pose_variance_normalized * 1.5)  # Weighted sum favoring pose metrics
                    ...
                    track_info[track_id] = {
                        ...
                        'translation_motion': translation_motion,
                        'pose_motion': pose_motion_total,
                        'pose_motion_norm': pose_motion_normalized,
                        'pose_variance': pose_variance_total,
                        'pose_variance_norm': pose_variance_normalized,
                    ...
                    logger.info(f"Track {track_id}: Frames={start_frame}-{end_frame} ({len(frame_ids)}), "
                               f"Size={avg_size:.4f}, Motion={total_motion:.4f}, "
                               f"Trans={translation_motion:.4f}, Pose={pose_motion_normalized:.4f}, "
                               f"PoseVar={pose_variance_normalized:.4f}, "
                               f"Density={motion_density:.4f}")
        
        # Step 1: Find potential main actor tracklets using a combination of size and motion metrics
        main_tracklets = []
        size_threshold = 0.6  # Consider tracklets with size at least 60% of the maximum
        ...
        if len(similar_size_tracklets) > 1:
            ...
            for track_id, info in similar_size_tracklets:
                # Normalize size and motion metrics to 0-1 scale
                norm_size = info['avg_size'] / max_size
                
                # Get maximum values across tracklets for normalization
                max_trans_motion = max([t_info['translation_motion'] for _, t_info in similar_size_tracklets]) if similar_size_tracklets else 1
                max_pose_motion = max([t_info['pose_motion_norm'] for _, t_info in similar_size_tracklets]) if similar_size_tracklets else 1
                max_pose_variance = max([t_info['pose_variance_norm'] for _, t_info in similar_size_tracklets]) if similar_size_tracklets else 1
                
                # Normalize individual motion components
                norm_trans_motion = info['translation_motion'] / max_trans_motion if max_trans_motion > 0 else 0
                norm_pose_motion = info['pose_motion_norm'] / max_pose_motion if max_pose_motion > 0 else 0
                norm_pose_variance = info['pose_variance_norm'] / max_pose_variance if max_pose_variance > 0 else 0
                
                # Combined motion score (higher weight to pose metrics)
                # Use total pose motion (frame-to-frame differences) as the key metric for identifying main actor
                # This captures dynamic movements like shooting, dribbling, etc.
                norm_motion = norm_pose_motion
                
                # Final score gives significant weight to pose motion when tracklets are of similar size
                combined_score = norm_size * 0.0 + norm_motion * 1.0
                combined_scores.append((track_id, combined_score, norm_size, norm_motion))
                
                logger.info(f"Track {track_id}: Size={norm_size:.4f}, Trans={norm_trans_motion:.4f}, "
                           f"Pose={norm_pose_motion:.4f}, PoseVar={norm_pose_variance:.4f}, "
                           f"Motion={norm_motion:.4f}, Combined={combined_score:.4f}")
            ...
            score_threshold = top_score * 0.7  # Consider tracklets with at least 70% of top score
        ...
        if not main_tracklets:
            # If no tracklets meet the criteria, just take the one with highest pose motion
            if track_info:
                largest_id = max(track_info.items(), key=lambda x: x[1]['pose_motion_norm'])[0]
                main_tracklets = [largest_id]
                logger.info(f"No tracklets met criteria, defaulting to highest pose motion: {largest_id}")
        ...
        if len(main_tracklets) > 1:
            ...
            for i in range(1, len(main_tracklets)):
                ...
                # Time threshold: allow gaps of up to 4 seconds (assuming 30fps)
            ...
            if continuous_groups:
                ...
                for group in continuous_groups:
                    ...
                    total_trans_motion = sum(track_info[tid]['translation_motion'] for tid in group)
                    total_pose_motion = sum(track_info[tid]['pose_motion_norm'] for tid in group)
                    total_pose_variance = sum(track_info[tid]['pose_variance_norm'] for tid in group)
                    avg_size = np.mean([track_info[tid]['avg_size'] for tid in group])
                    
                    # Combined score prioritizes duration, size and pose motion metrics
                    # Higher weight on pose motion for basketball/sports scenarios
                    # Focus on pose motion which captures the dynamic movements relevant to sports actions
                    #combined_score = total_frames * avg_size * (1 + 0.2*total_trans_motion + 0.4*total_pose_motion + 0.4*total_pose_variance)
                    # Only consider pose motion
                    combined_score = total_frames * avg_size * (1 + total_pose_motion)


                    group_scores.append((group, combined_score, total_frames, total_trans_motion, total_pose_motion))
                    
                    logger.info(f"Group {group}: Frames={total_frames}, Trans={total_trans_motion:.4f}, "
                               f"Pose={total_pose_motion:.4f}, PoseVar={total_pose_variance:.4f}, "
                               f"Size={avg_size:.4f}, Score={combined_score:.4f}")
                
                # Select group with highest score
                best_group, best_score, frames, trans_motion, pose_motion = max(group_scores, key=lambda x: x[1])
                logger.info(f"Selected best group with score {best_score:.4f}, {frames} frames, "
                           f"translation motion {trans_motion:.4f}, pose motion (primary selection criteria) {pose_motion:.4f}")
...
if __name__ == '__main__':
    ...
    #Take only rows where 'dance' or 'bouldering' is in the lowercase video name
    df          = df[df[0].str.lower().str.contains('iiith_soccer_004_2')]
